...
const popularOrder = ['jaipur', 'udaipur', 'varanasi', 'hampi', 'khajuraho'];
    ...
    .map(
      (destination) => `
        <button class="destination-button ${destination.id === selectedId ? 'active' : ''}" data-id="${destination.id}" type="button">
          <span class="destination-thumb" style="--thumb-x: ${destination.thumbX}; --thumb-y: ${destination.thumbY}"></span>
          <span>
